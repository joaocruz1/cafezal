...
export function DateRangePicker({ from, to, onChange, className = "" }: DateRangePickerProps) {
  ...
  return (
    <div className={`flex flex-wrap items-end gap-3 ${className}`}>
      <div className="w-40">
        <label htmlFor={fromId} className="mb-1 block text-sm font-medium text-stone-600">De</label>
        <div className="relative">
          <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-stone-400 pointer-events-none" />
          <input id={fromId} type="date" value={from} onChange={(e) => onChange(e.target.value, to)} className="w-full rounded-[var(--radius-md)] border border-stone-300 bg-white pl-9 pr-3 py-2 text-sm text-stone-800 focus:border-stone-500 focus:outline-none focus:ring-1 focus:ring-stone-500" />
        </div>
      </div>
      <div className="w-40">
        <label htmlFor={toId} className="mb-1 block text-sm font-medium text-stone-600">Ate</label>
        <div className="relative">
          <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-stone-400 pointer-events-none" />
          <input id={toId} type="date" value={to} onChange={(e) => onChange(from, e.target.value)} className="w-full rounded-[var(--radius-md)] border border-stone-300 bg-white pl-9 pr-3 py-2 text-sm text-stone-800 focus:border-stone-500 focus:outline-none focus:ring-1 focus:ring-stone-500" />
        </div>
      </div>
